...
describe("Pruebas para las funciones de manipulación de arrays", () => {
  ...
  describe("toUpperCaseStrings", () => {
    test("Convertir strings a mayúsculas en un array", () => {
      expect(toUpperCaseStrings(["hello", "world"])).toEqual([
        "HELLO",
        "WORLD",
      ]);
    });
  ...
  describe("sortNumbersAscending", () => {
    ...
    test("Ordenar números de forma ascendente con números negativos", () => {
      expect(sortNumbersAscending([-3, -1, -5, -2, -4])).toEqual([
        -5, -4, -3, -2, -1,
      ]);
    });
  ...
  describe("findFirstElement", () => {
    ...
    test("Encontrar el primer elemento que cumple una condición en array de objetos", () => {
      ...
      expect(findFirstElement(array, condition)).toEqual({
        name: "Bob",
        age: 30,
      });
    });
  ...
  describe("concatenateArrays", () => {
    ...
    test("Concatenar dos arrays de objetos", () => {
      expect(concatenateArrays([{ a: 1 }], [{ b: 2 }])).toEqual([
        { a: 1 },
        { b: 2 },
      ]);
    });
...
describe("Pruebas para la función agregarHabilidad", () => {
  ...
  test("Mantener las habilidades originales si no se agrega ninguna nueva", () => {
    ...
    expect(jugadorConHabilidad.habilidades).toEqual([
      "Ataque especial",
      "Defensa",
    ]);
  });
...
describe("filtrarPorAño", () => {
  ...
  test("filtra correctamente las películas con años negativos", () => {
    ...
    expect(filtrarPorAño(peliculas, -100)).toEqual([
      { titulo: "Pelicula2", año: -100 },
    ]);
  });
});
...
describe("calcularPromedioDuracionPorGenero", () => {
  test("calcula correctamente el promedio de duración de las películas por género", () => {
    expect(
      calcularPromedioDuracionPorGenero(peliculas, "Aventura")
    ).toBeCloseTo(135, 1);
  });
  ...
  test("maneja correctamente géneros con mayúsculas y minúsculas", () => {
    expect(calcularPromedioDuracionPorGenero(peliculas, "drama")).toBeCloseTo(
      100,
      1
    );
  });
...
describe("Vehiculo", () => {
  ...
  test("Método obtenerInformacion", () => {
    const vehiculo = new Vehiculo("Ford", "Fiesta", 2020);
    expect(vehiculo.obtenerInformacion()).toBe(
...
describe("Automovil", () => {
  ...
  test("Paso de parámetros con super", () => {
    ...
    expect(automovil.obtenerInformacion()).toBe(
      "Marca: Toyota, Modelo: Corolla, Año: 2022, Color: Rojo, Cilindrada: 2000, Potencia: 150, Puertas: 4, Asientos: 5, Transmisión: Automática"
    );
  });
});
...
describe("Motocicleta", () => {
  ...
  test("Paso de parámetros con super", () => {
    ...
    expect(motocicleta.obtenerInformacion()).toBe(
      "Marca: Honda, Modelo: CBR, Año: 2021, Color: Negro, Cilindrada: 1000, Potencia: 200, Ruedas: 2, Tipo: Deportiva"
    );
  });
});
...
describe("Camion", () => {
  ...
  test("Paso de parámetros con super", () => {
    ...
    expect(camion.obtenerInformacion()).toBe(
      "Marca: Volvo, Modelo: FH16, Año: 2020, Color: Blanco, Cilindrada: 16000, Potencia: 600, Ejes: 3, Capacidad de Carga: 40, Tipo de Carrocería: Remolque"
    );
  });
});
...
describe("Autobus", () => {
  ...
  test("Paso de parámetros con super", () => {
    ...
    expect(autobus.obtenerInformacion()).toBe(
      "Marca: Mercedes-Benz, Modelo: Tourismo, Año: 2019, Color: Azul, Cilindrada: 12000, Potencia: 400, Pasajeros: 50, Combustible: Diesel, Tipo de Motor: Trasero"
    );
  });
});
...
describe("Bicicleta", () => {
  ...
  test("Paso de parámetros con super", () => {
    ...
    expect(bicicleta.obtenerInformacion()).toBe(
      "Marca: Trek, Modelo: FX 2, Año: 2021, Tipo: Montaña, Marchas: 21, Material: Aluminio, Suspensión: Doble, Frenos: Disco, Tipo de Manubrio: Recto"
    );
  });
});
